...
def parse_feed(feed_id, max_age_hours=24):
    ...
    for entry in feed.entries[:50]:
        ...
        articles.append({
            "feed_id": feed_id, "source": feed_meta["source"], "tier": feed_meta["tier"],
            "title": title, "summary": summary[:500], "link": getattr(entry, "link", ""),
...
def scan_all_feeds(tiers=None, max_age_hours=24):
    ...
    return {
        "articles": all_articles, "ticker_mentions": ticker_counts,
        "top_tickers": top[:30],
...
def scan_ticker_feeds(ticker, max_age_hours=72):
    ...
    for entry in feed.entries[:20]:
        ...
        if pub_date and pub_date < cutoff:
            continue
        articles.append({
            "source": "Seeking Alpha", "ticker": ticker,
            "title": getattr(entry, "title", ""),
