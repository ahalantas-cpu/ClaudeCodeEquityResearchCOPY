...
class UsageTracker:
    ...
    def __init__(self):
        ...
        self._monthly_counts = defaultdict(int)
    ...
    def print_daily_report(self, target_date=None):
        ...
        for api_id, meta in API_REGISTRY.items():
            ...
            if meta.get("rate_limit_per_day"):
                ...
                pct = (calls / meta["rate_limit_per_day"]) * 100
                ...
                pct = (mo_calls / meta["rate_limit_per_month"]) * 100
            else:
        ...
        if warnings:
            ...
            for api_id, name, pct, cost in warnings:
                print(f"  {name}: {pct:.0f}% of free limit. Cost: {cost}")
            ...
            if u["calls"] > 0 or m["calls"] > 0:
                ...
                if max_pct >= 70:
                    summary["paid_tier_recommendations"].append({
                        "api_id": api_id, "name": meta["name"],
                        "usage_pct": round(max_pct, 1), "current_cost": meta["cost"],
